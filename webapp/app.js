...
function currentRunFrame() {
  const idx = Math.floor(state.runCycle / 60) % assets.run.length;
  return assets.run[idx];
}
function drawPlayer() {
  const p = state.player;
  const img = p.onGround ? currentRunFrame() : assets.jump;
  const runPhase = state.runCycle * 0.02;
  const bob = p.onGround && state.running ? Math.sin(runPhase * 2) * 2.2 : 0;
  const tilt = p.onGround && state.running ? Math.sin(runPhase) * 0.015 : 0;
