...
assets.run3.src = './assets/mascot-run-3.png';
assets.run4.src = './assets/mascot-run-4.png';
assets.jump.src = './assets/mascot-jump.png';
assets.obstacle.src = './assets/obstacle-candy.png';
assets.coin.src = './assets/coin.png';

const WORLD = { width: 420, height: 640, groundTop: 520, playerX: 74 };
...
const state = {
  ...
  obstacles: [],
  coinsList: [],
  clouds: Array.from({ length: 4 }, (_, i) => ({ x: 50 + i * 110, y: 72 + (i % 2) * 34, size: 36 + i * 6 })),
  stars: Array.from({ length: 18 }, (_, i) => ({ x: 20 + (i * 23) % 400, y: 28 + (i * 37) % 220, size: 1 + (i % 3), phase: i * 0.5 })),
  particles: [],
  player: { x: WORLD.playerX, y: WORLD.groundTop - 92, w: 54, h: 92, vy: 0, onGround: true, jumpBufferMs: 0 }
};
...
function spawnObstacle() {
  ...
  state.obstacles.push({ x: WORLD.width + 20, y: WORLD.groundTop - (tall ? 76 : 58), w: tall ? 34 : 38, h: tall ? 76 : 58, passed: false });
}
function spawnCoin() {
  state.coinsList.push({ x: WORLD.width + 16, y: WORLD.groundTop - (110 + Math.random() * 120), w: 22, h: 22, collected: false, spin: Math.random() * Math.PI });
}
function rectHit(a, b, insetX = 12, insetY = 8) {
  return a.x + insetX < b.x + b.w && a.x + a.w - insetX > b.x && a.y + insetY < b.y + b.h && a.y + a.h - insetY > b.y;
}
function coinHit(a, c) {
  return a.x + 12 < c.x + c.w && a.x + a.w - 12 > c.x && a.y + 10 < c.y + c.h && a.y + a.h - 10 > c.y;
}
function addCoinBurst(x, y) {
  for (let i = 0; i < 8; i++) {
    state.particles.push({ x, y, vx: (Math.random() - 0.5) * 3.2, vy: -Math.random() * 2.6 - 0.4, life: 28 + Math.random() * 12, size: 2 + Math.random() * 3 });
  }
}
...
function currentPlayerImage() {
  ...
  const frames = [assets.run1, assets.run2, assets.run3, assets.run4];
  const idx = Math.floor(state.frame / 5) % frames.length;
  return frames[idx];
}
function drawPlayer() {
  const p = state.player;
  const img = currentPlayerImage();
  const bob = p.onGround && state.running ? Math.sin(state.frame * 0.72) * 1.8 : 0;
...
async function loadLeaderboard() {
  try {
    ...
    leaderboardEl.innerHTML = payload.items.slice(0, 5).map((item) => `<div class="lb-row"><span>${item.rank}. ${item.name}</span><b>${item.score}</b></div>`).join('');
  } catch (error) {
