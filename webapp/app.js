...
assets.box.src = './assets/obstacle-box.png';
...
function spawnObstacle() {
  ...
  let type = 'box';
  if (typeRoll > 0.66) type = 'bucket';
  else if (typeRoll > 0.33) type = 'sign';

  const settings = {
    box: { w: 68, h: 52, offsetY: 0 },
    bucket: { w: 62, h: 76, offsetY: 0 },
    sign: { w: 66, h: 78, offsetY: 0 }
  }[type];
...
function obstacleHit(a, o) {
  let insetX = 16, insetY = 12;
  if (o.type === 'bucket') { insetX = 18; insetY = 12; }
  if (o.type === 'sign') { insetX = 16; insetY = 10; }
  if (o.type === 'box') { insetX = 18; insetY = 14; }
...
function update(deltaMs) {
  ...
  state.frame += t;
  state.runAnim += deltaMs * (0.006 + Math.min(state.speed, 10) * 0.00045);
...
function drawBackground() {
  ...
  overlay.addColorStop(1, 'rgba(255,245,251,0.10)');
  ctx.fillStyle = overlay;
  ctx.fillRect(0, 0, WORLD.width, WORLD.height);

  // darker visible horizon/surface line
  ctx.strokeStyle = 'rgba(116, 66, 104, 0.95)';
  ctx.lineWidth = 3;
  ctx.beginPath();
  ctx.moveTo(0, WORLD.floorY + 6);
  ctx.lineTo(WORLD.width, WORLD.floorY + 6);
  ctx.stroke();

  ctx.fillStyle = 'rgba(255, 192, 221, 0.16)';
  ctx.fillRect(0, WORLD.floorY - 8, WORLD.width, 12);
}
function currentRunBlend() {
  const frameFloat = state.runAnim % assets.run.length;
  const idx = Math.floor(frameFloat);
  const nextIdx = (idx + 1) % assets.run.length;
  const mix = frameFloat - idx;
  return { current: assets.run[idx], next: assets.run[nextIdx], mix };
}
function drawSpriteImage(img, x, y, h, alpha = 1) {
...
function drawPlayer() {
  ...
  const bob = p.onGround && state.running ? Math.sin(state.frame * 0.45) * 0.9 : 0;

  ctx.save();
  ctx.imageSmoothingEnabled = true;
  ctx.translate(p.x, p.y + bob);

  ctx.fillStyle = 'rgba(38, 22, 44, 0.20)';
  const shadowW = p.onGround ? 30 : 22;
  ctx.beginPath();
  ctx.ellipse(42, p.h + 8, shadowW, 7, 0, 0, Math.PI * 2);
  ctx.fill();

  const drawH = p.onGround ? 138 : 134;
  const offsetX = -22;
  const offsetY = -12;

  if (!p.onGround) {
    drawSpriteImage(assets.jump, offsetX, offsetY, drawH, 1);
  } else {
    const blend = currentRunBlend();
    drawSpriteImage(blend.current, offsetX, offsetY, drawH, 1 - blend.mix);
    drawSpriteImage(blend.next, offsetX, offsetY, drawH, blend.mix);
  }
...
function drawObstacle(o) {
  ...
  if (img.complete && img.naturalWidth > 0) {
    if (o.type === 'bucket') { dx -= 10; dy -= 16; dw = 82; dh = 98; }
    else if (o.type === 'sign') { dx -= 8; dy -= 8; dw = 78; dh = 92; }
    else if (o.type === 'box') { dx -= 8; dy -= 10; dw = 82; dh = 66; }
