...
assets.bg.src = './assets/store-bg-loop.jpg';
for (let i = 1; i <= 8; i++) {
  const img = new Image();
...
function obstacleHit(a, o) {
  ...
  if (o.type === 'box') { insetX = 14; insetY = 10; }
...
function drawBackground() {
  if (assets.bg.complete && assets.bg.naturalWidth > 0) {
    const imgW = Math.round(assets.bg.naturalWidth * (WORLD.height / assets.bg.naturalHeight));
    const shift = Math.floor(state.cameraX) % imgW;
    for (let i = -1; i < 3; i++) {
      const dx = i * imgW - shift;
      ctx.drawImage(assets.bg, dx, 0, imgW, WORLD.height);
    }
  } else {
  ...
  overlay.addColorStop(1, 'rgba(255,245,251,0.08)');
  ctx.fillStyle = overlay;
  ctx.fillRect(0, 0, WORLD.width, WORLD.height);

  // line of horizon / surface
  ctx.strokeStyle = 'rgba(255,255,255,0.65)';
  ctx.lineWidth = 2;
  ctx.beginPath();
  ctx.moveTo(0, WORLD.floorY + 8);
  ctx.lineTo(WORLD.width, WORLD.floorY + 8);
  ctx.stroke();

  ctx.fillStyle = 'rgba(255, 192, 221, 0.10)';
  ctx.fillRect(0, WORLD.floorY - 6, WORLD.width, 14);
}
...
function drawPlayer() {
  ...
  const bob = p.onGround && state.running ? Math.sin(state.frame * 0.7) * 1.2 : 0;
  ...
  ctx.beginPath();
  ctx.ellipse(38, p.h + 6, shadowW, 6, 0, 0, Math.PI * 2);
  ctx.fill();
  ...
  if (img.complete && img.naturalWidth > 0) {
    ...
    const offsetX = -18;
    const offsetY = -6;
    ctx.drawImage(img, 0, 0, frameW, frameH, offsetX, offsetY, drawW, drawH);
  } else {
...
function drawObstacle(o) {
  ...
  if (img.complete && img.naturalWidth > 0) {
    ...
    else if (o.type === 'box') { dx -= 6; dy -= 8; dw = 66; dh = 54; }
