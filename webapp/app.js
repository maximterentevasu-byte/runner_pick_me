...
assets.bg.src = './assets/store-background.jpg';
for (let i = 1; i <= 8; i++) {
  const img = new Image();
...
function obstacleHit(a, o) {
  ...
  if (o.type === 'puddle') { insetX = 18; insetY = 16; }
...
function update(deltaMs) {
  ...
  state.frame += t;
  state.cameraX += state.speed * t * 0.6;
...
function drawBackground() {
  if (assets.bg.complete && assets.bg.naturalWidth > 0) {
    const bgWidth = assets.bg.naturalWidth;
    const shift = Math.floor(state.cameraX) % bgWidth;
    ctx.drawImage(assets.bg, -shift, 0, bgWidth, WORLD.height);
    ctx.drawImage(assets.bg, bgWidth - shift, 0, bgWidth, WORLD.height);
  } else {
  ...
  overlay.addColorStop(1, 'rgba(255,245,251,0.16)');
  ctx.fillStyle = overlay;
  ctx.fillRect(0, 0, WORLD.width, WORLD.height);

  // floor highlight line
  ctx.fillStyle = 'rgba(255, 192, 221, 0.55)';
  ctx.fillRect(0, WORLD.floorY - 8, WORLD.width, 10);

  // subtle moving gloss on floor
  ctx.fillStyle = 'rgba(255,255,255,0.10)';
  for (let x = -40; x < WORLD.width + 80; x += 72) {
    const xx = x - (state.cameraX * 1.2 % 72);
    ctx.fillRect(xx, WORLD.floorY + 6, 34, WORLD.height - WORLD.floorY - 10);
  }
}
...
function drawPlayer() {
  ...
  const bob = p.onGround && state.running ? Math.sin(state.frame * 0.7) * 1.4 : 0;
  ...
  if (img.complete && img.naturalWidth > 0) {
    ...
    const drawH = p.onGround ? 116 : 114;
...
function drawObstacle(o) {
  ...
  if (img.complete && img.naturalWidth > 0) {
    if (o.type === 'bucket') {
      dx -= 8; dy -= 12; dw = 62; dh = 72;
    } else if (o.type === 'sign') {
      dx -= 4; dy -= 6; dw = 60; dh = 70;
    } else if (o.type === 'puddle') {
      dx -= 4; dy -= 4; dw = 74; dh = 26;
    }
